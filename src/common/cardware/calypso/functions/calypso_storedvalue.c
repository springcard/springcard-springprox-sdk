...
CALYPSO_PROC CalypsoCardStoredValueGetDecode(CALYPSO_CTX_ST* ctx, const BYTE get_sv_resp[], CALYPSO_SZ get_sv_respsize, signed long* balance, CALYPSO_STOREDVALUE_ST* target)
{
...
CALYPSO_PROC CalypsoCardStoredValueGetEx(CALYPSO_CTX_ST* ctx, BYTE type, BYTE resp[], CALYPSO_SZ* respsize)
{
	CALYPSO_RC rc;
...
CALYPSO_PROC CalypsoCardStoredValueGetForDebit(CALYPSO_CTX_ST* ctx, BYTE resp[], CALYPSO_SZ* respsize)
{
	return CalypsoCardStoredValueGetEx(ctx, CALYPSO_SV_DEBIT_TYPE, resp, respsize);
}

CALYPSO_PROC CalypsoCardStoredValueGetForReload(CALYPSO_CTX_ST* ctx, BYTE resp[], CALYPSO_SZ* respsize)
{
	return CalypsoCardStoredValueGetEx(ctx, CALYPSO_SV_RELOAD_TYPE, resp, respsize);
}

CALYPSO_PROC CalypsoCardStoredValueDebit(CALYPSO_CTX_ST* ctx, const BYTE get_sv_resp[], CALYPSO_SZ get_sv_respsize, signed long amount, WORD pdate, WORD ptime, BYTE resp[], CALYPSO_SZ* respsize)
{
	CALYPSO_RC rc;
	CALYPSO_SZ recv_len, card_send_len, sam_send_len;
	BYTE i;
	...
	if (sam_send_len != (CALYPSO_SZ)(ctx->Sam.Buffer[4] + 5))
	{
		/* Oups */
	...
	/* Check that there's no mistake... */
	if (card_send_len != (CALYPSO_SZ)(ctx->Card.Buffer[4] + 5))
	{
		/* Oups */
...
CALYPSO_PROC CalypsoCardStoredValueReload(CALYPSO_CTX_ST* ctx, const BYTE get_sv_resp[], CALYPSO_SZ get_sv_respsize, signed long amount, WORD pdate, WORD ptime, BYTE pfree[2], BYTE resp[], CALYPSO_SZ* respsize)
{
	CALYPSO_RC rc;
	CALYPSO_SZ recv_len, card_send_len, sam_send_len;
	BYTE i;
