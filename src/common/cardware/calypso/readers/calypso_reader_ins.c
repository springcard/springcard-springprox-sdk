...
CALYPSO_RC CalypsoCardSelectInnovatron(CALYPSO_CTX_ST* ctx, const BYTE atr[], CALYPSO_SZ atrlen)
{
...
CALYPSO_RC CalypsoCardSelectTcl(CALYPSO_CTX_ST* ctx, BYTE cid)
{
	if (ctx == NULL) return CALYPSO_ERR_INVALID_CONTEXT;
...
CALYPSO_RC CalypsoCardTransmit(CALYPSO_CTX_ST* ctx, const BYTE in_buffer[], WORD in_length, BYTE out_buffer[], WORD* out_length)
{
	CALYPSO_RC rc;
	WORD l = 0;
	...
	case PICC_TYPE_INNOVATRON:
		rc = Innovatron_Exchange(NULL, in_buffer, in_length, out_buffer, out_length);
#ifdef CALYPSO_OBEY_RCTIF
		if (rc == MI_QUIT)
		{
	...
	case PICC_TYPE_14443_A:
		rc = Tcl_Exchange(ctx->Card.Cid, in_buffer, in_length, out_buffer, out_length);
		break;
