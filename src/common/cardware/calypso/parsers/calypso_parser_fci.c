...
CALYPSO_RC CalypsoParseApplicationFciTpl(CALYPSO_CTX_ST *ctx, const BYTE tpl[], CALYPSO_SZ tplsize);
CALYPSO_RC CalypsoParseApplicationFciProp(CALYPSO_CTX_ST *ctx, const BYTE prop[], CALYPSO_SZ propsize);
CALYPSO_RC CalypsoParseApplicationFciDisc(CALYPSO_CTX_ST *ctx, const BYTE disc[], CALYPSO_SZ discsize);
 ...
 *   BYTE               fci[]          : the FCI
 *   BYTE              fcisize        : size of the atr
 *
 * RETURNS
 *   CALYPSO_RC
 *
 **/
CALYPSO_RC CalypsoParseFci(CALYPSO_CTX_ST *ctx, const BYTE fci[], CALYPSO_SZ fcisize)
{
  BYTE offset, length;
  const BYTE *value;
  WORD  tag;
